-- Open Get Key V1 
_G.Config = {
    CorrectKey = "key_2f84eb9bc5a64ada6d94", -- use Key Normal not key api
    UseKeySystem = true,

    KickOnWrongKey = false,
    KickDelay = 2,
    KickMessage = "Wrong Key! Please get the correct key.",

    KeyCheckAPI = {
        Enabled = false, -- Key Api
        URL = "https://discord.gg/V6YhUzyyuE",
        Method = "GET"
    },

    Title = "Tày Hub Key System Premium| v1",
    SubTitle = "Enter your key below to continue",
    PlaceholderText = "Enter Key Here...",

    GetKeyButtonText = "Tiktok",
    CheckKeyButtonText = "Check Key",
    DiscordButtonText = "Discord",
    TutorialButtonText = "Tutorial",

    KeyLink = "https://www.tiktok.com/@t.football14",
    DiscordLink = "https://discord.gg/V6YhUzyyuE",
    TutorialLink = "Trùm Skid",

    Images = {
        Background = "133093108462584",
        Character = "117730372760094",
        CharacterIcon = "104202858258775",
        GetKeyButton = "117928961549219",
        CheckKeyButton = "122513747232289",
        DiscordButton = "122513747232289",
        TutorialButton = "103629842357172"
    },
    ...
    Colors = {
        ...
        MainStroke = Color3.fromRGB(0, 255, 255),
        ...
        GetKeyButton = {
            Background = Color3.fromRGB(50, 100, 150),
            Stroke = Color3.fromRGB(70, 140, 200),
            StrokeHover = Color3.fromRGB(100, 180, 255)
        },
        CheckKeyButton = {
            Background = Color3.fromRGB(50, 100, 150),
            Stroke = Color3.fromRGB(70, 140, 200),
        ...
        DiscordButton = {
            ...
            Stroke = Color3.fromRGB(108, 121, 255),
            StrokeHover = Color3.fromRGB(138, 151, 255)
        },
        TutorialButton = {
            Background = Color3.fromRGB(150, 50, 150),
            Stroke = Color3.fromRGB(200, 70, 200),
            StrokeHover = Color3.fromRGB(255, 100, 255)
        }
    },

    Notifications = {
        KeyCopied = "Key link copied to clipboard!",
        DiscordCopied = "Discord link copied to clipboard!",
        TutorialCopied = "Tutorial link copied to clipboard!",
        CorrectKey = "Access granted!",
        IncorrectKey = "Incorrect Key! Try again.",
        NotificationTitle = "Tày Hub - Blox Fruit [Premium]"
    }
}

loadstring(game:HttpGet("https://raw.githubusercontent.com/Dex-Bear/Vxezehub/refs/heads/main/GetKeyFree.lua"))()

repeat wait() until _G.KeyVerified == true


-- ===== DELTA X COMPATIBILITY SHIMS =====
-- Ensure hookfunction works on Delta X (uses hookfunc alias)
if not hookfunction and hookfunc then
    hookfunction = hookfunc
elseif not hookfunction then
    hookfunction = function(f, r) return r end -- no-op fallback
end
-- Ensure newcclosure works
if not newcclosure then
    newcclosure = function(f) return f end
end
-- Ensure getnamecallmethod works
...
local function NotificacaoNightMystic(titulo, mensagem)
    local success = pcall(function()
        ...
        local LogoID = "rbxassetid://89906421818860"
    ...
    if not success then
        warn("[Moonlight Hub] Error notification")
    end
end

NotificacaoNightMystic("Moonlight Hub", "script has loaded successfully!")

-- ========================================
-- SAVE SYSTEM (Optimized)
-- ========================================
local HttpService = Services.HttpService
local FolderName = "Moonlight Hub Hub"
...
-- Delta X compatible hookfunction wrapper
...
G.Pos = function(I, e)
		local target = PosMon and (typeof(PosMon) == "CFrame" and PosMon.Position or PosMon) or Root.Position
		return (Root.Position - target).Magnitude <= e
	end;
G.Dist = function(I, e)
...
BringEnemy = function()
    ...
    local targetPos = PosMon and (typeof(PosMon) == "CFrame" and PosMon.Position or PosMon) or hrp.Position
    ...
    for _, mob in ipairs(enemies) do  
        ...
        if hum and root and hum.Health > 0 and not IsRaidMob(mob) then  
            ...
            if dist > 5 and dist <= _G.BringRange and not root:GetAttribute("Tweening") then
                count += 1  
...
end) -- end pcall Delta X namecall hook 1
...
QuestNeta = function()
	...
	-- Delta X: retry loadstring up to 5 times on failure
	local Library
	...
	for _attempt = 1, 5 do
		...
		if _ok and _result then
			...
			warn("[Moonlight Hub Hub] UI load attempt " .. _attempt .. " failed, retrying...")
			task.wait(2)
		end
	end
	if not Library then
		error("[Moonlight Hub Hub] Failed to load UI after 5 attempts. Check your internet or executor.")
	end
	Library = Library:MakeWindow({
    Title = "Moonlight Hub Hub | Blox fruit",
    SubTitle = "Thais and kien",
    SaveFolder = "Moonlight Hub.json"
})
-- Criar ScreenGui
local screenGui = Instance.new("ScreenGui")
...
imageButton.Image = "rbxassetid://89906421818860"
...
uiStroke.Color = Color3.fromRGB(128,0,0) -- Magenta/Rosa forte
...
Discord:AddDiscordInvite({
    ...
    Logo = "rbxassetid://74394297758710",
    Invite = "https://discord.gg/WF2sPbv3GD"
})
local credits = Discord:AddParagraph({
    Title = "Credits For Someone Peoples",
    Desc = ""
})
credits:SetDesc("Support @thaideptraisyntrax on Discord - Sever Moonlight Hub")

-- ======================= PHẦN SHOP MUA MELEE (THAY THẾ) =======================
        ...
        task.spawn(function()
            while _G.AutoBuyMelee do
                ...
                if not data or not data.Pos then
                    -- Thông báo lỗi (nếu có hàm notify)
                ...
                if lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if dist > 15 then
                        ...
                        -- Một số melee cần gọi thêm (như Dragon Breath)
...
-- (Các phần Shop khác như Sword, Gun, Abilities, Misc vẫn giữ nguyên)
...
spawn(function()
    while wait() do
        if getgenv().Join then
            local input = tostring(getgenv().Job or "")
            local prefix, code = string.match(input, "^(.-)%-(.+)$")
            if code and #code > 20 then
                pcall(function()
                    game:GetService("TeleportService"):TeleportToPrivateServer(
                        game.PlaceId, code, {game.Players.LocalPlayer}
                    )
                end)
            else
                pcall(function()
                    game:GetService("TeleportService"):TeleportToPlaceInstance(
                        game.PlaceId, input, game.Players.LocalPlayer
                    )
                end)
            end
        end
...
Status:AddButton({
    ...
    Callback = function()
        ...
        local prefix, code = string.match(input, "^(.-)%-(.+)$")
        if code and #code > 20 then
            -- Private Server Link Code (vd: BananaCat-U2Vjd...)
            pcall(function()
                game:GetService("TeleportService"):TeleportToPrivateServer(
                    game.PlaceId,
                    code,
            ...
            -- JobId thuong (vd: 3293f114-0bc9-43b7-...)
            pcall(function()
                game:GetService("TeleportService"):TeleportToPlaceInstance(
                    game.PlaceId,
                    input,
...
NotificacaoNightMystic("Moonlight Hub", "✅All systems have been downloaded!")
